...
z2 <- read.zoo(textConnection(L), index = list(1, 2), sep=",", 
       header = TRUE, FUN  = f2)
...
z3 <- read.zoo(textConnection(L), sep = ",", header = TRUE, 
	index = list(1, 2), tz = "", format = "%d.%m.%Y %H:%M")
