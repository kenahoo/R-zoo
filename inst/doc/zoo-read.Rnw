...
z <- read.zoo(textConnection(Lines), header = TRUE, index = 1:2,
...
# use chron appending seconds and index = 1:2

f <- function(d, t) {
       chron(d, paste(t, "00", sep = ":"), format = c("m.d.y", "h:m:s"))
}
z <- read.zoo(textConnection(L), index = 1:2, sep=",", header = TRUE, FUN  = f)


# Example 7b
# alternative.  as.chron

f2 <- function(d, t) as.chron(paste(d, t), format = "%d.%m.%Y %H:%M")
z2 <- read.zoo(textConnection(L), index = 1:2, sep=",", header = TRUE, 
	FUN  = f2)

# alternative using POSIXct instead of chron
# Note that the two index columns are pasted together since FUN was not
# specified and passed to as.POSIXct(x, tz = "", format = "%d.%m.%Y %H:%M")
# since tz and format were.

z3 <- read.zoo(textConnection(L), sep = ",", header = TRUE, 
...
library(zoo)
z1 <- read.zoo(textConnection(Lines1), header = TRUE, index = 1:2, tz = "")
...
z <- read.zoo(textConnection(L), skip = 1,
       FUN = paste, FUN2 = as.chron, index = 3:4)
...
library(zoo)
library(chron)
z <- read.zoo(textConnection(Lines), header = TRUE, index = 1:2, FUN = chron)
