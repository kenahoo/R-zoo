...
\Abstract{
  ...
  function expects either a text file (or more generally a text connection)
  as input or a data frame (and the former case is handled by first using \fct{read.table}
  to produce the data frame). Subsequently, \fct{read.zoo} provides a wide
...
\textbf{Strategy:} No transformation of time tindex needed, hence
