...
%\VignetteDepends{zoo,tseries,fCalendar,fSeries,strucchange,DAAG}
...
\code{"its"} in package \pkg{its} \citep{zoo:its:2004}
...
versions of \pkg{fCalendar}, Section~\ref{sec:fCalendar} has a somewhat outdated example how
...
<<rollapply>>=
rollapply(Z, 5, sd)
rollapply(Z, 5, sd, fill = TRUE, align = "left")
@
...
\pkg{fCalendar} \Sexpr{gsub("-", "--", packageDescription("fCalendar")$Version)},
