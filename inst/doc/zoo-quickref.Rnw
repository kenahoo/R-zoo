...
time(tmp) <- as.integer(time(tmp) + 0.0001) + Epoch

# find index of last value in each one second interval
ix <- !duplicated(time(tmp), fromLast = TRUE)

# merge with grid and add back the attributes which were lost when we
# used as.integer
range. <- range(time(tmp))
merge(tmp[ix], zoo(, seq(range.[1], range.[2], "sec")))
