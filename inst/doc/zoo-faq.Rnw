...
There are 64 other packages that depend on, suggest, can use with or 
...
\pkg{cxxPack} & R/C++ Tools for Literate Statistical Practice (outdated) \\
\pkg{delftfews} & timeseries functions used at Nelen en Schuurmans \\
...
\pkg{RghcnV3} & Process GHCN V3 climate series \\
\pkg{hydroTSM} & Time series for hydrological modelling \\
...
\pkg{RcppTemplate} & \pkg{Rcpp} \proglang{R}/\proglang{C++} object mapping library and package template (outdated) \\
\pkg{sandwich} & Robust covariance matrix estimators \\
\pkg{sde} & Simulation and Inference for Stochastic Differential Equations \\
\pkg{solaR} & Solar radiation and performance of photovoltaic systems \\
\pkg{spacetime} & classes and methods for spatio-temporal data \\
...
\pkg{termstrc} & Zero-coupon Yield Curve Estimation \\
...
\pkg{TShirtQuote} & Time series database interface extensions for \code{get.hist.quote} \\
\pkg{VhayuR} & \proglang{R} interface to the Vhayu time series database \\
\pkg{wq} & Exploring water quality monitoring data \\
...
\pkg{hydroGOF} & Compare goodness-of-fit of simulated and observed hydrological series \\
\pkg{latticeExtra} & \pkg{grid}-based graphics: use with \code{xyplot.zoo} \\ 
\pkg{MeDiChI} & Model-based deconvolution of genome-wide binding (ChIP-chip) data \\
\pkg{mondate} & Keep track of dates in month units. \\
\pkg{playwith} & Interactive graphics: works with \code{xylot.zoo} \\ 
...
\pkg{TSAgg} & Time series aggregation for incomplete time series data \\
...
\pkg{UsingR} & A collection of datasets to accompany the textbook ``Using \proglang{R} for Introductory Statistics'' \\
\pkg{Zelig} & Everyone's statistical software \\ \hline
\end{tabular}

\begin{tabular}{|p{3.6cm}|p{11cm}|} \hline
\multicolumn{2}{|l|}{\emph{Enhances}} \\ \hline
\pkg{chron} & Chronological objects which can handle dates and times \\ 
\pkg{lubridate} & Make dealing with dates a little easier \\ 
