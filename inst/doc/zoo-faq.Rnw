...
There are 52 other packages that depend on, suggest, can use with or 
...
spaced series filling in the missing times and
...
the index class, it is typically necessary to fix up the class as shown here:
...
In this approach we fill in the missing values with linear
approximations rather than \code{NA}s:

<<fillin-approx>>=
# using z from above
tseq <- seq(start(zym), end(zym), by = 1/12)
na.approx(zym, xout = tseq)
@

\item{\code{merge.zoo}}  
This approach also fills in the missing spots with \code{NA}s.  It
...
a bit of care in case the tseq values are slightly different than the 
index values of \code{z} since the matching will then not work.   For example,
with \pkg{chron} times this approach has problems since the times in the data
and the times in the sequence produced are slightly off.  That is the
following does NOT work as intended:

<<fillin-merge-bad>>=
# does not work properly!!!
# since index(z) values not all in tseq due to small differences
...
cat("*** Wrong due to slight differences in time(z) and tseq ***\n")
merge(z, zoo(, tseq))
@

We can fix this up by rounding off the values to the nearest second:
<<fillin-merge-2>>=
Rnd <- function(x) {
       time(x) <- chron(trunc(time(x), "sec"))
       x
}
merge(Rnd(z), Rnd(zoo(, tseq)))
@

or by adding a \code{MATCH.times} method which redefines matching so that 
values within 1 second are regarded as the same.

<<fillin-merge-3>>=
# rnd function is from above
MATCH.times <- function(x, table, nomatch = NA, ...) {
       match(trunc(x, "sec"), trunc(table, "sec"), nomatch = nomatch, ...)
}
merge(z, zoo(, tseq))
@

The \code{as.ts} approach or the \code{na.approx} 
approach work here without any special measures at all:
