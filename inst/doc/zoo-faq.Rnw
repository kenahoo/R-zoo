...
zoo has extended \code{origin} argument of \code{as.Date.numeric} so that it
has a default of \code{origin="1970-01-01"} (whereas in the core of R it has no 
default and must always be specified).  Also zoo supports 
\code{origin = "0000-00-00"} to handle
Matlab style dates.  Note that these are strictly upwardly compatible 
